...
portability. WSL/Windows and macOS are the supported platforms for the full
Hermes Pets overlay experience. Native Linux remains an investigation target
until launch, overlay, and verification paths are proven end to end.
...
| macOS | Git checkout run with `PYTHONPATH=src python3 -m hermes_pet.cli`; package overlay and managed Hermes plugin are the source of truth | Supported | Supported: local Python bridge at `127.0.0.1:17473`, TUI plugin and Electron overlay connect to `ws://127.0.0.1:17473` | Supported through native Electron `src/hermes_pet/overlay/src/main.js`; `overlay-status` and `close` use native process discovery | Verified on real Mac hardware | Full supported platform |
...
available, but Phase 5 does not claim native Linux support for them without
dedicated checkout/runtime rehearsal evidence.
...
Supported macOS overlay behavior means all of the following pass on verified
real Mac hardware:
