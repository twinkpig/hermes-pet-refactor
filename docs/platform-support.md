...
| --- | --- | --- | --- | --- | --- | --- |
| WSL2 on Windows 10/11 with Windows interop | Git checkout run with `PYTHONPATH=src python -m hermes_pet.cli`; package overlay is the source of truth | Supported | Supported: Electron built-in WS Server at `0.0.0.0:17473`, TUI connects to `ws://172.27.128.1:17473` | Supported through `powershell.exe` and `src/hermes_pet/overlay/scripts/launch-windows-overlay.ps1` | Supported with `scripts/verify-live-overlay.sh` when Windows interop is available | Full supported platform |
...
| macOS | Git checkout run with `PYTHONPATH=src python3 -m hermes_pet.cli`; package overlay is the source of truth | Experimental | Experimental: local Python bridge at `127.0.0.1:17473`, Electron overlay connects to `ws://127.0.0.1:17473` | Experimental through native Electron `src/hermes_pet/overlay/src/main.js`; `overlay-status` and `close` use native process discovery | Manual smoke testing | Experimental overlay support |
